...
var author$project$Main$emptyModel = {
	newTaskModel: author$project$Main$emptyNewTaskModel,
	tasks: _List_fromArray(
		[
			A7(author$project$Main$Task, 'Test Task', 'Test', 1, 0, elm$core$Maybe$Nothing, (12 * 60) * 60, 1),
			A7(author$project$Main$Task, 'Test Task 2', 'Test', 2, 0, elm$core$Maybe$Nothing, (12 * 60) * 60, 1)
		]),
	time: 0,
	uid: 3
};
...
var author$project$Main$logErr = F2(
	function (label, r) {
		if (r.$ === 'Ok') {
			var value = r.a;
...
var author$project$Main$taskDecoder = A8(
	...
	elm$json$Json$Decode$maybe(
		A2(elm$json$Json$Decode$field, 'lastDone', elm$json$Json$Decode$int)),
	A2(elm$json$Json$Decode$field, 'nextDue', elm$json$Json$Decode$int),
	A2(elm$json$Json$Decode$field, 'doneCount', elm$json$Json$Decode$int));
var elm$json$Json$Decode$list = _Json_decodeList;
var elm$json$Json$Decode$map2 = _Json_map2;
var author$project$Main$storageModelDecoder = A3(
...
var author$project$Main$init = function (flags) {
	var model = A2(
		...
		A2(author$project$Main$StorageModel, _List_Nil, 0),
		A2(
			author$project$Main$logErr,
			'Json Decode',
	...
	return _Utils_Tuple2(
		_Utils_update(
			...
			{tasks: model.tasks, time: flags.time, uid: model.uid}),
	...
	return author$project$Main$periodToMillis(
		A2(
			...
			A2(
				...
				elm$core$String$toLower(model.newTaskModel.period))));
};
var author$project$Main$newTask = function (model) {
	return {
		...
		nextDue: model.time,
		period: author$project$Main$getPeriodMillis(model),
...
var author$project$Main$storeTasks = _Platform_outgoingPort(
	'storeTasks',
	function ($) {
							...
							return elm$json$Json$Encode$object(
								_List_fromArray(
									[
										...
										_Utils_Tuple2(
										'doneCount',
										elm$json$Json$Encode$int($.doneCount)),
										_Utils_Tuple2(
										'id',
										...
										_Utils_Tuple2(
										'nextDue',
										elm$json$Json$Encode$int($.nextDue)),
										_Utils_Tuple2(
										'period',
										elm$json$Json$Encode$int($.period)),
										_Utils_Tuple2(
										'tag',
		...
		switch (msg.$) {
			...
			case 'Do':
				...
				var updateTask = function (t) {
					return _Utils_eq(t.id, id) ? _Utils_update(
						t,
						{
							doneCount: t.doneCount + 1,
							lastDone: elm$core$Maybe$Just(model.time),
							nextDue: model.time + t.period
						}) : t;
				};
				var newTasks = A2(elm$core$List$map, updateTask, model.tasks);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{tasks: newTasks}),
					author$project$Main$storeTasks(
						A2(author$project$Main$StorageModel, newTasks, model.uid)));
			...
			default:
				var newTasks = _Utils_ap(
					...
					_List_fromArray(
						[
							author$project$Main$newTask(model)
						]));
				var newTaskModel = model.newTaskModel;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{newTaskModel: author$project$Main$emptyNewTaskModel, tasks: newTasks, uid: model.uid + 1}),
					author$project$Main$storeTasks(
						A2(author$project$Main$StorageModel, newTasks, model.uid + 1)));
		}
	});
var author$project$Main$isDueSoon = F3(
	function (period, now, task) {
		return _Utils_cmp(now + period, task.nextDue) > 0;
	});
	...
	function (period, now, task) {
		return A2(
			...
			A2(
				elm$core$Maybe$map,
				function (t) {
					return _Utils_cmp(now - period, t) < 0;
				},
				task.lastDone));
	});
...
var author$project$Main$newTaskView = function (model) {
	...
	var findUnit = A2(
		elm$core$Result$withDefault,
		1,
		A2(elm$parser$Parser$run, elm$parser$Parser$int, model.newTaskModel.period));
	var addS = F2(
		function (unit, str) {
			...
			[
				...
				_List_fromArray(
					[
						...
						elm$html$Html$Attributes$value(model.newTaskModel.description),
				...
				_List_fromArray(
					[
						...
						elm$html$Html$Attributes$value(model.newTaskModel.tag),
				...
				_List_fromArray(
					[
						...
						elm$html$Html$Attributes$value(model.newTaskModel.period),
				...
				_Utils_ap(
					A2(periodOptions, findUnit, model.newTaskModel.period),
					A2(periodOptions, findUnit + 1, model.newTaskModel.period)))
			]));
};
...
var author$project$Main$viewTask = F2(
	function (time, todo) {
		var recentlyDone = A3(author$project$Main$isRecentlyDone, (12 * 60) * 60, time, todo);
		return A2(
			elm$html$Html$li,
			_List_Nil,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							elm$html$Html$Attributes$class('task-view')
						]),
					_List_fromArray(
						[
							A2(
							...
							_List_fromArray(
								[
									...
									recentlyDone ? 'task-done' : 'task-todo'),
									elm$html$Html$Events$onClick(
									author$project$Main$Do(todo.id))
								]),
							_List_fromArray(
								[
									elm$html$Html$text(todo.description + (' - ' + todo.tag))
								]))
						]))
				]));
	});
...
var author$project$Main$viewKeyedTask = F2(
	function (time, todo) {
		return _Utils_Tuple2(
			elm$core$String$fromInt(todo.id),
			A3(elm$html$Html$Lazy$lazy2, author$project$Main$viewTask, time, todo));
	});
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2(
						elm$core$List$map,
						author$project$Main$viewKeyedTask(time),
						tasks))
				]));
	});
...
var author$project$Main$view = function (model) {
	var recentlyDone = A2(author$project$Main$isRecentlyDone, (12 * 60) * 60, model.time);
	var dueSoon = A2(author$project$Main$isDueSoon, (12 * 60) * 60, model.time);
		...
		_List_fromArray(
			[
				...
				A2(
					...
					A2(elm$core$List$filter, isVisible, model.tasks))),
				elm$html$Html$text(
				elm$core$String$fromInt(model.time) + (' ' + elm$core$String$fromInt(
					author$project$Main$getPeriodMillis(model))))
			]));
};
