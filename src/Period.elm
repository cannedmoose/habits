module Period exposing (Period(..), addToPosix, minusFromPosix, toMillis, toString, periodParser, decoder, encode, parse)
...
toMillis period =
  case period of
    Minutes i -> (i*60*1000)
    Hours i -> (i*60*60*1000)
    Days i -> (i*24*60*60*1000)
    Weeks i -> (i*7*24*60*60*1000)
    Months i -> (i*28*24*60*60*1000)
...
toString period =
    case period of
      Minutes i -> (addS i "Minute")
      Hours i -> (addS i "Hour")
      Days i -> (addS i "Day")
      Weeks i -> (addS i "Week")
      Months i -> (addS i "Month")
...
periodUnitParser amount = Parser.oneOf
